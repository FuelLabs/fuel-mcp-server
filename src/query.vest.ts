...
vi.mock('@xenova/transformers', async (importOriginal) => {
    const actual = await importOriginal<typeof import('@xenova/transformers')>();
    return {
        ...actual,
...
describe('queryDocs (Qdrant)', () => {
    ...
    beforeEach(() => { 
        ...
        (QdrantClient as any).mockImplementation(() => ({ // Use type assertion here just in case
            search: mockSearchFn,
            getCollection: mockGetCollectionFn,
        }));
    ...
    it('should query Qdrant successfully with default parameters', async () => {
        ...
        expect(mockSearchFn).toHaveBeenCalledWith(
            process.env.QDRANT_COLLECTION || "bun_qdrant_docs", 
    ...
    it('should throw error if Qdrant client initialization fails', async () => {
        ...
        (QdrantClient as any).mockImplementation(() => { 
            throw initError;
        });
    ...
    it('should throw error if collection does not exist', async () => {
        const collectionError = new Error("Not Found") as any;
