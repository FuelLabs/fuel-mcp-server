...
import { chunkMarkdown, type MarkdownChunk } from '../common/chunker.js';
...
async function getOrCreateQdrantCollection(client: QdrantClient, collectionName: string): Promise<void> {
    try {
        ...
        if (!collectionExists) {
            log(`Collection '${collectionName}' not found. Creating...`);
            ...
            log(`Collection '${collectionName}' already exists.`);
        }
    } catch (error) {
...
export async function indexDocsQdrant(
    ...
    if (markdownFiles.length === 0) {
        log("No markdown files found in the specified directory. Exiting.");
    ...
    try {
        ...
        log(`Successfully ensured collection '${collectionName}' exists.`);
    } catch (error) {
    ...
    let embedder: any;
    try {
        log(`Loading embedding model '${modelName}'... (This might take a while the first time)`);
        ...
        try {
            ...
            if (chunks.length > 0) {
                ...
                allChunks.push(...chunks.map((chunk, index) => ({
                 ...
                 log(`  - No chunks generated (file might be empty or only contain whitespace).`);
            }
        } catch (error) {
    ...
    if (allChunks.length === 0) {
        log("No content chunks were generated from the markdown files.");
        return;
    }

    log(`Total chunks generated: ${allChunks.length}. Preparing for embedding and adding to Qdrant...`);

    // Process Chunks in Batches: Embed, Upsert to Qdrant
    for (let i = 0; i < allChunks.length; i += BATCH_SIZE) {
        const batch = allChunks.slice(i, i + BATCH_SIZE);
        log(`Processing batch ${Math.floor(i / BATCH_SIZE) + 1} of ${Math.ceil(allChunks.length / BATCH_SIZE)} (size: ${batch.length})...`);
        ...
        try {
            ...
            log(`  - Generated ${batchEmbeddings.length} embeddings.`);
        ...
        try {
            ...
            if (points.length === 0) {
                log("  - No valid points to upsert in this batch.");
                continue;
            }

            // Log point details before upsert attempt
            log(`  - Preparing to upsert batch ${Math.floor(i / BATCH_SIZE) + 1} with ${points.length} points.`);
            ...
            log(`  - Batch upserted successfully (${points.length} points).`);
        } catch (error) {
    ...
    log(`--------------------------------------------------`);
    log(`Qdrant Indexing finished!`);
    log(`Total markdown files processed: ${markdownFiles.length}`);
    log(`Total chunks upserted to Qdrant: ${totalChunksProcessed}`);
    log(`--------------------------------------------------`);
}
