...
mock.module('node:fs/promises', () => ({
    readdir: mock(async (dirPath) => {
        if (dirPath === './test-docs-empty')
            return [];
        if (dirPath === './test-docs-no-md')
            return ['not-a-markdown.txt'];
        if (dirPath === './test-docs')
            return ['doc1.md', 'doc2.md', 'image.png'];
        if (dirPath === './test-docs-large') {
            return Array.from({ length: 150 }, (_, i) => `large_doc_${i + 1}.md`);
        }
        throw new Error(`ENOENT: no such file or directory, scandir '${dirPath}'`);
    }),
    readFile: mock(async (filePath, encoding) => {
...
const pipelineMock = mock(async (task, model) => {
...
const mockQdrantClientInstance = {
    getCollections: mock(async (options) => {
    ...
    createCollection: mock(async (name, params) => {
...
describe('indexDocsQdrant', () => {
    ...
    it('should use environment variables for configuration', async () => {
        ...
        const collectionLogFound = consoleCalls.some(args => typeof args[0] === 'string' && args[0].includes(`Collection: ${expectedCollectionName}`));
